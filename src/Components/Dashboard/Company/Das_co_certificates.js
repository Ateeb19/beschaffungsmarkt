...
const Das_co_certificates = () => {
    ...
    const handle_add_product = () => {
        if (data.is_premium === 0) {
            showAlert(
                <>
                    < FaExclamationTriangle className="me-2 fs-2 text-warning" /> Please upgrade your membership to upload certificates.
                </>, "warning"
            )
    ...
    const handle_product = async () => {
        ...
        try {
            const res = await axios.post(`${Backend_URL}/api/certificates/add`, formData, {
    ...
    const handle_edit_product = async () => {
        ...
        formData.append("certificateId", edit_product._id);
        formData.append("title", product_title);
        if (image_changed_product) {
            if (selectedFile_product) {
                formData.append("image", selectedFile_product);
        ...
        try {
            const res = await axios.post(`${Backend_URL}/api/certificates/edit`, formData, {
                headers: { "Content-Type": "multipart/form-data" },
                withCredentials: true,
            });
            showAlert(res.data.msg, "success")
    ...
    const handle_product_delete_confirm = async () => {
        try {
            const res = await axios.post(`${Backend_URL}/api/certificates/delete`, {
                certificateId: selected_product._id,
            }, {
                withCredentials: true,
            })
    ...
    return (
        ...
        //         <button className="add-product-button" onClick={handle_add_product}><MdAdd /> Add Certificate</button>
        //     </div>
